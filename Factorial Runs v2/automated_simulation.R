      ...
      if(factorial[i,j]== "+" & colnames(factorial[i,j])=="RT drift"){ 
        for(k in 1:length(sample_data)){
          ...
          if(sample_data[[k]]$peptide[1] %in% out_peptides){
            sample_data[[k]]$RT <- cbind(peptide= rep(sample_data[[k]]$peptide[1],sim.size),
                                         sample_data[[k]]$RT+runif(1,-2,2)*IQR(sample_data[[k]]$RT), 
                                         sample_data[[k]]$TotalArea, 
                                         sample_data[[k]]$MassAccu, 
                                         sample_data[[k]]$FWHM,
                                         RESPONSE= c("FAIL"))
            sample_data[[k]] <- sample_data_k
          }
      ...
      if(factorial[i,j]== "+" & colnames(factorial[i,j])=="Total Area drift"){ 
        for(k in 1:length(sample_data)){
          ...
          if(sample_data[[k]]$peptide[1] %in% out_peptides){
            sample_data[[k]]$RT <- cbind(peptide= rep(sample_data[[k]]$peptide[1],sim.size),
                                         sample_data[[k]]$RT, 
                                         sample_data[[k]]$TotalArea + runif(1,-2,2)*IQR(sample_data[[k]]$TotalArea), 
                                         sample_data[[k]]$MassAccu, 
                                         sample_data[[k]]$FWHM,
                                         RESPONSE= c("FAIL"))
            sample_data[[k]] <- sample_data_k
        ...
        for(k in 1:length(sample_data)){
          ...
          if(sample_data[[k]]$peptide[1] %in% out_peptides){
            sample_data[[k]]$RT <- cbind(peptide= rep(sample_data[[k]]$peptide[1],sim.size),
                                         sample_data[[k]]$RT, 
                                         sample_data[[k]]$TotalArea, 
                                         sample_data[[k]]$MassAccu + runif(1,-2,2)*IQR(sample_data[[k]]$MassAccu), 
                                         sample_data[[k]]$FWHM,
                                         RESPONSE= c("FAIL"))
            sample_data[[k]] <- sample_data_k
          }
        ...
        for(k in 1:length(sample_data)){
          ...
          if(sample_data[[k]]$peptide[1] %in% out_peptides){
            sample_data[[k]]$RT <- cbind(peptide= rep(sample_data[[k]]$peptide[1],sim.size),
                                         ...
                                         sample_data[[k]]$TotalArea, 
                                         sample_data[[k]]$MassAccu, 
                                         sample_data[[k]]$FWHM + runif(1,-2,2)*IQR(sample_data[[k]]$FWHM),
                                         RESPONSE= c("FAIL"))
            sample_data[[k]] <- sample_data_k
          }
