simulate_step_shift_partial <-function(guide.set.scale, sim.size, rand.pep){
...
####Randomly select half of the peptides
ind<-sort(sample(1:num.pep, rand.pep))
#ind<-sort(sample(1:num.pep, ceiling(num.pep/2)))

for(j in 1:num.pep){ 
  Data<-c()
  sample_data <- sample_density(guide.set.scale,guide.set.scale$peptide[j], sim.size*32)
  
  Data<-data.frame(idfile=1:(32*sim.size),
                   peptide=rep(levels(guide.set.scale$peptide)[j], (32*sim.size)),
...
for(j in ind){
  ...
  for(i in 1:sim.size){
    Data<-rbind(Data,c(i,rep(levels(guide.set.scale$peptide)[j],1),
                       sample_data[i,1]+beta*IQR(sample_data[,1]), 
                       sample_data[i,2],
                       sample_data[i,3],
                       sample_data[i,4]))
  }
...
Data1<-add_features(guide.set.scale, Data1, ind)
Data1[[j]][,1]<-(16*sim.size+1):(16*sim.size+sim.size)

for(j in ind){Data.set[[j]][(16*sim.size+1):(16*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){
  ...
  for(i in 1:sim.size){
    Data<-rbind(Data,c(i,rep(levels(guide.set.scale$peptide)[j],1),
                       sample_data[i,1], 
                       sample_data[i,2]+beta*IQR(sample_data[,2]),
                       sample_data[i,3],
                       sample_data[i,4]))
  }
  Data<- as.data.frame(Data,stringsAsFactors = F)
...
for(j in ind){Data.set[[j]][(17*sim.size+1):(17*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){
  ...
  for(i in 1:sim.size){
    Data<-rbind(Data,c(i,rep(levels(guide.set.scale$peptide)[j],1),
                       sample_data[i,1], 
                       sample_data[i,2],
                       sample_data[i,3]+beta*IQR(sample_data[,3]),
                       sample_data[i,4]))
  }
  Data<- as.data.frame(Data,stringsAsFactors = F)
  for (i in c(1,3:ncol(Data))){ Data[,i]<-as.numeric(Data[,i])}
...
for(j in ind){Data.set[[j]][(18*sim.size+1):(18*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(19*sim.size+1):(19*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(20*sim.size+1):(20*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(21*sim.size+1):(21*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(22*sim.size+1):(22*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(23*sim.size+1):(23*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(24*sim.size+1):(24*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){
  ...
  for(i in 1:sim.size){
    Data<-rbind(Data,c(i,rep(levels(guide.set.scale$peptide)[j],1),
                       ...
                       sample_data[i,2], 
                       sample_data[i,3],
                       sample_data[i,4]+beta*IQR(sample_data[,4])))
  }
  Data<- as.data.frame(Data,stringsAsFactors = F)
  for (i in c(1,3:ncol(Data))){ Data[,i]<-as.numeric(Data[,i])}
  colnames(Data)<-c("idfile", "peptide", colnames(sample_data))
...
for(j in ind){Data.set[[j]][(25*sim.size+1):(25*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(26*sim.size+1):(26*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(27*sim.size+1):(27*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(28*sim.size+1):(28*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(29*sim.size+1):(29*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(30*sim.size+1):(30*sim.size+sim.size),]<-Data1[[j]]}
...
for(j in ind){Data.set[[j]][(31*sim.size+1):(31*sim.size+sim.size),]<-Data1[[j]]}
